       ...
       */
        ...
        */
        ...
        computeDestinationPoint: function(start, distance, bearing, radius) {
            ...
            var smallDelta = Number(distance) / radius; // angular distance in radians
            var Theta = Number(bearing).toRad();

            var smallPhi1 = Number(lat).toRad();
            var Lambda1 = Number(lng).toRad();

            var smallPhi2 = Math.asin( Math.sin(smallPhi1)*Math.cos(smallDelta) +
                Math.cos(smallPhi1)*Math.sin(smallDelta)*Math.cos(Theta) );
            var Lambda2 = Lambda1 + Math.atan2(Math.sin(Theta)*Math.sin(smallDelta)*Math.cos(smallPhi1),
                    Math.cos(smallDelta)-Math.sin(smallPhi1)*Math.sin(smallPhi2));
            Lambda2 = (Lambda2+3*Math.PI) % (2*Math.PI) - Math.PI; // normalise to -180..+180°

            return {
                latitude: smallPhi2.toDeg(),
                longitude: Lambda2.toDeg()
            };
